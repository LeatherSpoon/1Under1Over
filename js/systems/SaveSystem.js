...
export class SaveSystem {
  ...
  apply(data) {
    ...
    // v8+ systems — deserialize, then re-apply PP-side bonuses explicitly
    // (onDefeat/onComplete callbacks fire only on live events, not on load)
    if (bosses) {
      if (data.bosses) bosses.deserialize(data.bosses);
      bosses.applyBonuses();
    }
    if (expedition && data.expedition) expedition.deserialize(data.expedition);
    if (challenges) {
