...
export class Environment {
  ...
  // `name`/`greeting` make the NPC talkable: main.js shows a "[E/ACT] Talk to
  // <name>" hint within range and pops the greeting as an npc-bark toast.
  _addNpc(glbKey, x, z, { scale = 1, rotY = 0, r = 0.45, name = null, greeting = null } = {}) {
    const group = new THREE.Group();
    group.position.set(x, 0, z);
    group.rotation.y = rotY;
    this.group.add(group);
    const npc = { group, glbKey, scale, mixer: null, hasModel: false, name, greeting };
    this._npcs.push(npc);
  ...
  /** Ambient NPCs in the current zone (for the talk-prompt sweep in main.js). */
  getNpcs() { return this._npcs; }
