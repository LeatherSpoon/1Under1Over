...
// GLB replacements for specific material types — preloaded once, cloned per node.
// Falls back to the procedural primitive shape below if not loaded yet.
...
export class ResourceNode {
  ...
  _buildMesh() {
    ...
    if (glbSrc) {
      ...
      this.group.add(mesh);
      this._addIndicator(visual.height);
      return;
    }
  ...
  // Floating label indicator, shared by both the procedural and GLB mesh paths
  _addIndicator(height) {
    const indicatorGeo = new THREE.SphereGeometry(0.08, 6, 4);
    const indicatorMat = createToonMaterial(0x00ffcc);
    const indicator = new THREE.Mesh(indicatorGeo, indicatorMat);
    indicator.position.y = height + 0.6;
