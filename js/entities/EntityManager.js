...
export class EntityManager {
  constructor(scene, onAggro) {
    ...
    // Optional predicate set from main.js — spawns returning false are skipped
    // (used to keep defeated zone bosses from respawning).
    this.spawnFilter = null;
  }

  spawnForZone(enemySpawns, nodeSpawns) {
    if (this.spawnFilter) enemySpawns = enemySpawns.filter(this.spawnFilter);
    this._inCombat = false;
