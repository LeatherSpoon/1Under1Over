...
4. Bump **Last updated**; prune so the file stays about one screen (session log ≤10 lines) — anything AGENTS.md or git history already records doesn't belong there
...
The **Mine is the realm portal hub**. Realm zones such as Verdant Maw, Lagoon Coast, Frozen Tundra, and Depths should return to `mine` (labelled "Mine Hub" or "Return to Mine"), not directly to `landingSite`. Keep `MINE_ZONE_PORTALS` inside the visible mine floor (`CONFIG.GROUND_SIZE`) so every hub portal has background/floor under it, and update `tests/mineLayout.test.js` when changing hub geometry.
...
| Mine grid + portal hub layout (procedural) | `js/scene/zones/Mine/layout.js` (`js/scene/MineLayout.js` re-exports for legacy/tests) |
