...
## Art round: camera + forest (2026-07-23)

- **Camera pitch 69° → 46°** (`CAMERA_OFFSET` (0,18,7) → (0,14,13.5)) — owner asked for a less top-down read so trees/props show in profile; 39° was tested and rejected (occlusion stacking, too much world depth in frame). Angle swept live in-page before committing.
- **Forest = reference-matched Rodin trees** — owner supplied three reference images (broadleaf, windswept, spruce); image-to-3D matched them closely where text prompts had drifted. Pool: broadleaf 40% / oak 25% / spruce 20% / windswept 15%. Text-prompt-era variants E/F/G retired same day they shipped (owner: "look like lumps"); A–C hand-built trees retired ("older trees should not be present").
- **Outline convention for organic meshes**: baked `*_OutlineHull` shells, never runtime scale-hulls (black speckle poke-through) and never voxel-remesh shells (bridge canopy gaps into black slabs).
- **Hull shells are smooth cages that err inward (2026-07-23)** — the round-3 displace+decimate shells still speckled (owner: black specks mid-canopy; TreeI hull lost its limbs to collapse-decimate). Root cause is geometric: on a surface whose concave detail is finer than the shell offset, any strictly-outside shell must slash through bumps. Recipe now (`Assets/3D/LandingProps/hull_rebake.py`): Laplacian-smooth the copy ~20 rounds into a low-frequency cage, re-inflate by a graph-blurred `(offset − signed distance)` field, flip. Bump peaks poke through the shell — invisible mid-body, sketchy notches at the rim — instead of the shell flecking the body. Verified in-game at 46°: canopies clean, windswept tree's branches and blob islands all inked.
- **Hull v8: only provably-ink faces survive (2026-07-23, round 6)** — owner re-flagged residual specks (red circles) + "tree separated from trunk" (black circles). Cage now also drops floating micro-components (<300 verts; a caged 10-vert Rodin debris shard = floating black dot — TreeH had two), cuts up-facing panels (normal z > 0.78 can never be silhouette at the 46° camera), and deletes every face not strictly outside the body (7-sample BVH test) — specks now geometrically impossible, rim ink gains hand-inked notches.
- **TreeI was 12 disconnected pieces — bridged with real branches (2026-07-23)** — the Rodin windswept mesh's six canopy islands floated free of the trunk (owner: "appears separated from the trunk"). Fixed at the foundation, per owner's read: six tapered bezier branch tubes (wood-brown, auto-anchored trunk→island centroid, arcing up) joined into the body mesh; hull covers them, so branches carry ink like the reference. Analysis/bridging never round-trips the body mesh (UV/normal safety); wisps <300 verts stay in the body but get no cage.
- **Bold tier ink, clean crown, per-tree weight (2026-07-24, owner mock)** — owner proposed leaning in: thicker deliberate outline arcs around canopy tiers, with the crown top guaranteed free of black. Hull v10: max-propagated inflation field + overshoot (continuous fat bands), penetration cut strict only in the top ~28% of height (zero black up top; sub-pixel slivers tolerated below to keep bands whole), hemisphere-visibility cut kills lower-tier ink peeking through canopy gaps, cut masks majority-smoothed. Ink weight is per-tree (`PER_TREE` in `hull_rebake.py`): fat 0.042+0.012 on single-mass crowns (broadleaf/oak), slim 0.030/0.028 on multi-lobed trees (windswept blobs/spruce tiers — the fat band drowned them in black collars). Hard-won veto: never repair a cage by per-vertex pushing against the bumpy source surface — it shatters the smooth envelope into visible shards (failed twice).
- **TreeH2 = cavity-ink variant; ink signal is distance-to-envelope, not AO (2026-07-24)** — owner asked for a duplicate attempting deliberate interior tier lines (liked TreeJ's). Cycles AO failed three ways (16-sample noise → stipple; leaf-scale pits drown tier creases at any distance; smooth-proxy selected-to-active bake still stippled). Working signal: **distance from body surface to the smooth containing envelope** — large exactly where the envelope bridges a recess (tier creases, blob seams), smooth and tier-scale by construction; rasterized per-vertex → UV by hand (`Assets/3D/LandingProps/cavity_ink.py`), percentile-banded two-level ink (crisp line + penumbra) multiplied into a COPY of the diffuse (original stashed in a material custom prop — reruns never double-ink). Shipped as `Ghibli_Tree_H2.glb` placed at the Landing Site path tree for A/B vs plain H; pool untouched pending owner verdict. Geometric alternatives tried and dropped: tighten-envelope (blur dilutes localized pulls; broadleaf creases too shallow for spruce-style arcs). Gotcha: never leave Cycles as the render engine — it ignores backface culling, so every inverted hull renders as a solid black blob.
- **TreeH hull = strictly-containing envelope; ink only at perceived edges (2026-07-24)** — owner's spec after v10 still showed marks: bold outline, zero artifacts inside it from ANY perspective, concave areas included; keep the model, new procedure. v11 (`hull_envelope.py`): voxel-remesh → smooth → lift-to-clearance along the envelope's own normals, no face deletions. A smooth containing envelope with clearance > chord error cannot cross the body, so interior artifacts are geometrically impossible from every angle (verified at 46°/62°/25° + in-game). Trade accepted: the interior tier-overlap arcs from the cage era are gone — clean interior won over drawn tier lines. Root-cause find: Rodin meshes carry flipped-normal patches, so normal-signed distance tests lie (this poisoned every earlier "crosser" count) — inside/outside now decided by unsigned distance + multi-ray escape vote. Old cage hull archived in the blend (`TreeH_OutlineHull_v10_archived`), per owner: do not delete.
