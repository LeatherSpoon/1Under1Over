...
Encounters happen in the 3D world but resolve in a **breakaway window** (Pokémon flow: meet it, the fight opens its own screen, you return). Resolution is **NGU-style time-based**; **no dodge/execution mechanic** (Locked). Gear sets your stats; Threads can speed timers; wins pay materials plus creature data — which feeds blueprints, so *killing a creature teaches its counter*. The live creature roster and archetypes port as encounter content; the live stat-resolution engine likely survives mostly intact inside the window.
...
- **M0 — Vocabulary re-anchor.** Renames only: Recompile→Offload, Archive Data→Data, Compute→Threads. No mechanics change; makes every later diff read in 8.1 language.
