...
- **Skills/stats under 8.1.** The live game's 14 stats + trainers, and the previously planned skill web, are NOT assumed. Stats likely hang off gear + implants (cyborg), but this needs my ruling before any code work.
- **The multiplier chain.** PP stays the run engine; whether the live multiplier-chain structure survives as-is is undecided.
- **Al's identity.** Suggested: Al is (or becomes) the computer's mind. Flavor call, mine to make.
...
- **The pool's final name.** Threads is dead for good (owner, 2026-08-02). Ichor is the standing name — better, but I'm still not completely satisfied. Keep auditioning; don't bake any name into UI or code identifiers until I settle it. (The narration contingency stands regardless of which name wins.)
