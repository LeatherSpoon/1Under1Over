...
test('tech tree defines the four starter branches plus research nodes', () => {
  assert.deepEqual(
    TECH_NODES.map(n => n.id),
    [
      'fieldFabrication', 'droneLogistics', 'terrainControl', 'biomeAccess',
      'efficientMining', 'deepVeins', 'swiftHarvest', 'materialFocus',
      'combatChip', 'armorCoating', 'energyEfficiency'
    ]
  );
});
